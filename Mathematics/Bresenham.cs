...
namespace Mathematics
{
    ...
    public static class Bresenham
    {
        ...
        /// <param name="to">End coordinate.</param>
        /// <returns>List of points on the line between both coordinates</returns>
        public static List<Vector3D> PlotLine(Vector3D from, Vector3D to)
        {
            if (Math.Abs(to.Y - from.Y) < Math.Abs(to.X - from.X))
            {
                return from.X > to.X ? PlotLineLow(to, from) : PlotLineLow(from, to);
        ...
        /// <param name="to">End coordinate.</param>
        /// <returns>List of points on the line between both coordinates</returns>
        private static List<Vector3D> PlotLineLow(Vector3D from, Vector3D to)
        {
            ...
            for (var i = from.X; i <= to.X; i++)
            {
                var point = new Vector3D { X = i, Y = y };
                lst.Add(point);
        ...
        /// <param name="to">End coordinate.</param>
        /// <returns>List of points on the line between both coordinates</returns>
        private static List<Vector3D> PlotLineHigh(Vector3D from, Vector3D to)
        {
            var lst = new List<Vector3D>();
            ...
            for (var i = from.Y; i < to.Y; i++)
            {
                var point = new Vector3D { X = x, Y = i };
                lst.Add(point);
