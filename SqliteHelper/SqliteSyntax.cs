...
namespace SqliteHelper
{
    internal sealed class SqliteSyntax
    {
        ...
        internal bool AdvancedSyntaxCheck(ICollection tableInfo, IEnumerable<TableSet> table, TableSet row, TableColumns convert)
        {
            ...
            {
                if (!CheckNullability(rows, convert)) return false;
                if (!CheckTypeCompatibility(rows, convert)) return false;
                if (!CheckUniqueness(row, convert)) return false;
            }
        ...
        {
            if (rows != null || convert.NotNull) return true;
        ...
        {
            if (!convert.PrimaryKey && !convert.Unique) return true;
