...
namespace CommonLibrary.Tests
{
    ...
    public sealed class IoFileHandler
    {
        ...
        public async Task GetNewFileName()
        {
            ...
            // Assert that the first file name matches the expected pattern robustly
            Assert.AreEqual("IO(0).txt", Path.GetFileName(info));
            Assert.IsTrue(info.Contains(@"IoFileHandler\GetNewFileName"), $"Actual path was: {info}");
            ...
            // FIXED: Assert that the second file name matches the expected pattern robustly
            Assert.AreEqual("IO(1).txt", Path.GetFileName(info));
            Assert.IsTrue(info.Contains(@"IoFileHandler\GetNewFileName"), $"Actual path was: {info}");
